/*
  ...
  1. New Tables
    - `follows` - Core follow relationships between users
    - `follow_requests` - Pending follow requests for private accounts
    - `user_blocks` - Blocked user relationships
    - `follow_activity_log` - Rate limiting and audit trail

  2. Security
    - Enable RLS on all tables
    - Add policies for user access control
    - Implement rate limiting constraints
    - Add audit logging for follow actions
...
CREATE TABLE IF NOT EXISTS follow_activity_log (
  ...
  action text NOT NULL CHECK (action IN ('follow', 'unfollow', 'block', 'unblock')),
